...
def main_dashboard(page: str = None):
    ...
    with ui.dialog() as debug_dialog, ui.card().classes('w-full max-w-4xl h-[80vh] bg-slate-900 border border-slate-700'):
        with ui.row().classes('w-full justify-between items-center border-b border-slate-700 pb-2'):
            ui.label('System Logs').classes('text-lg font-mono text-green-400')
            with ui.row():
                ui.button(icon='refresh', on_click=lambda: log_content.refresh()).props('flat dense round')
                ui.button(icon='close', on_click=debug_dialog.close).props('flat dense round')
        
        with ui.column().classes('w-full h-full overflow-auto font-mono text-xs p-2') as log_container:
            @ui.refreshable
            def log_content():
                ...
                for log in reversed(list(mem_handler.logs)):
                    ...
                    elif log['level'] == 'WARNING': color = 'text-yellow-400'
                    
                    with ui.row().classes('w-full gap-2 hover:bg-slate-800/50 p-1 rounded'):
                        ui.label(log['ts']).classes('text-slate-500 min-w-[60px]')
                        ui.label(log['level']).classes(f'font-bold min-w-[50px] {color}')
    ...
    async def refresh_all_data():
        try:
            ...
            ui.notify(f"Sync Failed: {str(e)}", type='negative', position='bottom-right')
