...
class EmailNotifier:
    ...
    def __init__(self):
        ...
        if not self.enabled:
            logger.warning("Email notifications not configured (EMAIL_FROM or EMAIL_PASSWORD missing)")
        else:
    ...
    def send_alert_email(self, alert_type: str, message: str, 
    ...
    def send_price_alert_email(self, symbol: str, price: float, 
    ...
    def send_order_notification_email(self, order_id: str, symbol: str, 
    ...
    def send_risk_alert_email(self, alert_level: str, message: str, 
                             recommendations: Optional[List[str]] = None) -> bool:
        """Send risk management alert email"""
        subject = f"⚠️ Risk Alert - {alert_level}"
        
        color = '#d32f2f' if alert_level == 'HIGH' else '#ff9800' if alert_level == 'MEDIUM' else '#4caf50'
...
def send_email_alert(alert_type: str, message: str, data: Optional[Dict] = None) -> bool:
    """
...
if __name__ == '__main__':
    ...
    if notifier.enabled:
        ...
        print("Email not configured. Set EMAIL_FROM and EMAIL_PASSWORD environment variables.")
        print("\nFor Gmail:")
