...
class OrderSchema(Schema):
    ...
    order_type = fields.Str(
        required=True,
        validate=validate.OneOf(['MARKET', 'LIMIT', 'SL', 'SL-M'])
    )
    ...
    product = fields.Str(
        required=True,
        validate=validate.OneOf(['INTRADAY', 'DELIVERY', 'MTF'])
    )
...
class SymbolSchema(Schema):
    ...
    exchange = fields.Str(
        validate=validate.OneOf(['NSE', 'BSE', 'NFO', 'BFO', 'MCX'])
    )
...
class AlertRuleSchema(Schema):
    ...
    alert_type = fields.Str(
        required=True,
        validate=validate.OneOf(['PRICE', 'VOLUME', 'RSI', 'MACD'])
    )
