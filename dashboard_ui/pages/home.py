...
def stats_widget(state):
    ...
    with ui.row().classes('w-full gap-4 flex-wrap'):
        Components.kpi_card('Total Value', pf.get('total_value', 0), delta=0.5, prefix='₹')
...
def render_gainers_losers_widget():
    ...
    with Components.card():
        with ui.row().classes('w-full items-center justify-between mb-2'):
            ...
            ui.button(icon='refresh', on_click=lambda: load_active_tab()).props('flat dense round color=slate-400')
        ...
        pagination_area = ui.row().classes('w-full justify-between items-center mt-2 px-2')
        ...
        def render_table(rows_page):
            ...
            with content_area:
                if not rows_page:
                    ui.label('No data available.').classes('text-slate-500 italic p-4 text-center w-full')
                    return
                ...
                with ui.element('table').classes('w-full text-right text-xs'):
                    ...
                    with ui.element('tbody'):
                        for r in rows_page:
                            color = 'text-green-400' if r['pct_change'] >= 0 else 'text-red-400'
                            bg_hover = 'hover:bg-slate-800/50 transition-colors'
                            
                            with ui.element('tr').classes(f'border-b border-slate-800/30 {bg_hover}'):
                                # Symbol
                                ...
                                # Change
                                with ui.element('td').classes('py-2 font-mono ' + color):
                                    ui.label(f"{r['pct_change']:+.2f}%")
                                # Volume
                                with ui.element('td').classes('py-2 text-slate-500 hidden sm:table-cell pr-2'):
        ...
        def update_pagination_view():
            ...
            with pagination_area:
                ui.label(f"Showing {start+1}-{min(end, total_items)} of {total_items}").classes('text-xs text-slate-500')
                ...
                with ui.row().classes('gap-1'):
                    ...
                    ui.button(icon='chevron_left', on_click=lambda: change_page(-1)).props('flat dense round').bind_enabled_from(state_tracker, 'page', backward=lambda p: p > 0)
                    ui.button(icon='chevron_right', on_click=lambda: change_page(1)).props('flat dense round').bind_enabled_from(state_tracker, 'page', backward=lambda p: p < max_page)
        ...
        async def load_active_tab():
            ...
            try:
                ...
                if not data:
                    cnt.clear()
                    with cnt:
                         ui.label('Data service returned empty result.').classes('text-red-400 text-center w-full')
                    return
                ...
                cnt.clear()
                with cnt:
                    ui.label(f'Error loading data: {str(e)}').classes('text-red-400 text-sm font-mono p-2')
                    print(f"Movers Error: {e}", exc_info=True)

        # Controls (Toggle)
        with ui.row().classes('w-full justify-center -mt-2 mb-2'):
            tgl = ui.toggle(['Gainers', 'Losers'], value='Gainers').props('no-caps dense spread outline color=indigo')
