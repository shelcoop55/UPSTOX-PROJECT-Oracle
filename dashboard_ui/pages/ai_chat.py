...
def render_page(state):
    Components.section_header('AI Assistant', 'Ask Gemini about your portfolio and the market', 'smart_toy')
    ...
    with ui.column().classes('w-full h-[calc(100vh-200px)] gap-0 border border-slate-700 rounded-lg overflow-hidden'):
        
        # 1. Chat Area (Scrollable)
        chat_container = ui.column().classes('w-full flex-grow p-4 overflow-y-auto space-y-4 bg-slate-900/50')
        
        # 2. Input Area (Fixed at bottom)
        with ui.row().classes('w-full p-4 bg-slate-800 border-t border-slate-700 gap-2 items-center'):
            input_field = ui.input(placeholder='Ask about "Gainers" or "Holdings"...').classes('flex-grow').props('rounded outlined dense dark')
            send_btn = ui.button(icon='send', color='indigo').props('round dense flat')
            
        def add_message(text, is_user=True):
            with chat_container:
                align = 'ml-auto bg-indigo-600 text-white' if is_user else 'mr-auto bg-slate-700 text-slate-200'
        ...
        # Welcome Message
        add_message("👋 Hi! I'm your Upstox AI Assistant. Ask me anything about the market or your portfolio.", is_user=False)
