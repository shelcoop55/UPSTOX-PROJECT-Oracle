...
def render_page(state):
    Components.section_header('Option Chain', 'Professional Live Options Chain', 'list_alt')
    ...
    with ui.row().classes('w-full gap-4 items-center bg-slate-900 p-4 rounded mb-4'):
        ...
        # Symbol Selector
        ui.select(['NIFTY', 'BANKNIFTY', 'FINNIFTY', 'RELIANCE', 'HDFCBANK', 'INFY', 'TCS', 'SBIN'], 
                  label='Instrument', value=selected_symbol['val'], 
                  on_change=lambda e: update_expiries(e.value)).props('outlined dense dark options-dense').classes('w-40')
        
        # Expiry Selector
        expiry_select = ui.select(expiry_options['val'], label='Expiry', value=selected_expiry['val']).props('outlined dense dark options-dense').classes('w-40')
        
        # Refresh Button
        ui.button(icon='refresh', on_click=lambda: load_chain()).props('flat round dense').tooltip('Refresh Data')
    ...
    async def update_expiries(symbol):
        ...
        dates = await asyncio.to_thread(service.get_expiry_dates, service._get_instrument_key(symbol))
    ...
    def render_chain_table(data):
        ...
        with ui.grid(columns=cols).classes('w-full gap-1 text-center font-mono text-xs bg-slate-900 p-2 rounded-t sticky top-0 z-10'):
        ...
        # Rows
        with ui.grid(columns=cols).classes('w-full gap-y-1 gap-x-0 items-center text-center font-mono text-sm'):
            for s in strikes:
                ...
                with ui.element('div').classes(f'contents {bg_class}'):
                    ...
                    # STRIKE (Center)
                    ui.label(f"{strike_price}").classes('font-bold bg-slate-800 py-1 text-white border-x border-slate-700')
