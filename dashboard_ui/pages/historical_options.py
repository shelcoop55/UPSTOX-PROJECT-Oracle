...
def historical_options_page():
    ...
    with ui.column().classes('w-full p-4 space-y-4'):
        ...
        ui.label('Download expired option chains from Upstox Historical API').classes('text-gray-600')
        ...
        if IMPORT_ERROR:
             with ui.card().classes('w-full bg-red-100 border-l-4 border-red-500 p-4'):
                 ...
                 ui.label(f'Could not import backend scripts: {IMPORT_ERROR}').classes('text-red-600 font-mono text-sm')
                 ui.label('Please check that commands are run from the project root and requirements are installed.').classes('text-red-600 text-xs mt-1')
        ...
        with ui.card().classes('w-full p-4 space-y-4'):
            with ui.row().classes('w-full justify-between items-start'):
                ...
                # --- NEW: Drill Down Link ---
                ui.button('Drill Down / Inspector', on_click=lambda: ui.open('/?page=api_debugger', new_tab=True)).props('flat dense icon=travel_explore color=blue').tooltip('Open Advanced Debugger')

                # Log Toggle
                log_view = ui.log().classes('w-full h-32 bg-slate-900 text-xs font-mono text-green-400 p-2 rounded hidden')
                ui.switch('Show Logs', on_change=lambda e: log_view.classes(remove='hidden') if e.value else log_view.classes(add='hidden'))
            ...
            with ui.row().classes('items-end gap-4'):
                ...
                fetch_btn = ui.button('Fetch Expiries', icon='search').props('color=primary')
        ...
        async def on_fetch_click():
            ...
            if '|' in symbol:
                ...
                if 'NSE_FO' in symbol:
                     ui.notify("⚠️ You entered a specific Contract Key (NSE_FO), not an Underlying.", type='warning', timeout=5000)
                     ui.notify("This page downloads Chains. For specific contracts, use 'Drill Down' -> 'Direct Key Mode'.", type='info', timeout=7000)
                ...
                # Case 2: Valid Underlying Keys (NSE_INDEX, NSE_EQ)
                     ...
                     ui.notify("⚠️ Unknown Key Format. Expecting NIFTY, BANKNIFTY or NSE_INDEX|...", type='warning')
            ...
            try:
                ...
                if '|' in symbol:
                    ...
                    with expiry_container:
                        ui.separator()
                        ui.label(f'Target: {symbol}').classes('text-xl font-mono text-green-400 font-bold')
                        
                        # Just show "Download" config (Simplified)
                        with ui.card().classes('w-full bg-slate-900 border border-green-700'):
                             with ui.row().classes('gap-4 items-end'):
                                 d_days = ui.select({'0': 'Expiry Day', '5': 'Last 5 Days', '30': 'Last 30 Days'}, value='5', label='History Depth').props('dark')
                                 d_int = ui.select(['1minute', '30minute', 'day'], value='30minute', label='Interval').props('dark')
                             ...
                             async def do_single_download():
                                 ...
                                 try:
                                     ...
                                     end_d = datetime.strptime(symbol.split('|')[-1], '%d-%m-%Y')
                                     start_d = end_d - timedelta(days=days)
                                     
                                     from_s = start_d.strftime('%Y-%m-%d')
                                     to_s = end_d.strftime('%Y-%m-%d') # Upstox historic uses YYYY-MM-DD in URL path
                                     
                                     candles = await run.io_bound(fetch_expired_historical_candles, symbol, d_int.value, from_s, to_s)
                                     
                                     if candles:
                                         log_view.push(f"SUCCESS: Got {len(candles)} candles.")
                                         ...
                                         ui.download(output.getvalue().encode('utf-8'), fname)
                                         ui.notify('Download Started', type='positive')
                                     else:
                                         log_view.push("No data found (Empty Response). Check contract liquidity.")
                                         
                                 except Exception as e:
                                     log_view.push(f"Error: {e}")

                             ui.button('Download CSV', on_click=do_single_download, icon='download').props('color=green')
                ...
                log_view.push(f"Fetch completed in {duration:.2f}s. Found {len(expiries) if expiries else 0} expiries.")
                
                if not expiries:
                    ui.notify(f'No expiries found for {symbol}. (Check logs)', type='warning')
                    ...
                    try:
                        with open("access_token.txt", "r") as f:
                            tok = f.read().strip()
                            log_view.push(f"Current Token (First 10 chars): {tok[:10]}...")
                    except:
                ...
                with expiry_container:
                    ui.separator()
                    ui.label(f'Step 2: Select Expiries for {symbol}').classes('text-lg font-bold mt-2')
                    
                    # Selection controls
                    with ui.row().classes('gap-4 mb-2'):
                        ui.button('Select All', on_click=lambda: set_all_checkboxes(True)).props('size=sm outline')
                        ui.button('Deselect All', on_click=lambda: set_all_checkboxes(False)).props('size=sm outline')
                    ...
                    with ui.scroll_area().classes('h-64 border border-gray-200 rounded p-2'):
                    ...
                    if len(expiries) > 50:
                        ui.label(f"Showing newest 50 of {len(expiries)} expiries").classes('text-xs text-gray-400')
                    ...
                    ui.label('Step 3: Download Options').classes('text-lg font-bold mt-2')
                    
                    with ui.row().classes('gap-4 items-center'):
                         chk_save_local = ui.checkbox('Save Locally (CSV)', value=True)
                         chk_save_db = ui.checkbox('Save to Database', value=False)
                    
                    with ui.expansion('Candle Configuration (OHLC)', icon='candlestick_chart').classes('w-full border border-slate-700 rounded'):
                         with ui.column().classes('p-2 gap-2'):
                             chk_download_candles = ui.checkbox('Download OHLC Candles', value=False)
                             with ui.row().classes('gap-4'):
                                 sel_interval = ui.select(['1minute', '30minute', 'day'], value='30minute', label='Interval')
                                 sel_days = ui.select({'0': 'Expiry Day Only', '5': 'Last 5 Days', '30': 'Last 30 Days'}, value='0', label='History Depth')
                             
                             ui.label('⚠️ Warning: Downloading candles for 150+ contracts may take time and hit API limits.').classes('text-xs text-orange-400')


                    async def start_download():
                        selected = [cb.text for cb in expiry_checkboxes if cb.value]
                        if not selected:
                            ui.notify('Please select at least one expiry date', type='warning')
                            return
                        
                        if not chk_save_local.value and not chk_save_db.value:
                            ui.notify('Please select at least one save method', type='warning')
                            return

                        ui.notify(f'Starting download for {len(selected)} expiries...', type='info')
                        ...
                        for i, expiry in enumerate(selected):
                            try:
                                ...
                                ui.notify(f"Processing {expiry}...", type='ongoing', timeout=1000)
                                log_view.push(f"Processing {expiry}...")
                                
                                # 1. Fetch Option Chain
                                raw_contracts = await run.io_bound(fetch_expired_option_contracts, symbol, expiry)
                                if not raw_contracts:
                                    results.append({"expiry": expiry, "status": "failed", "msg": "No data returned"})
                                ...
                                try:
                                    ...
                                    start_date_obj = exp_date_obj - timedelta(days=days_back if days_back > 0 else 0)
                                ...
                                log_view.push(f"  Found {total_contracts} contracts. Starting processing...")
                                
                                for idx, c in enumerate(raw_contracts):
                                    # Update UI every 10 items to show responsiveness
                                    if idx % 10 == 0: await asyncio.sleep(0.01)
                                        ...
                                        if inst_key:
                                            ...
                                            if candles:
                                                ...
                                                if chk_save_db.value:
                                                     await run.io_bound(store_expired_candles, inst_key, sel_interval.value, candles)
                                                
                                                # Attach to object for CSV (Top 5 candles summary?)
                                                if chk_save_local.value:
                                                    # Flatten simple stats
                                                    opt['candle_open_start'] = candles[0][1] if candles else ''
                                                    opt['candle_close_end'] = candles[-1][4] if candles else ''
                                            else:
                                ...
                                if chk_download_candles.value:
                                    log_view.push(f"  Fetched candles for {contracts_with_candles}/{total_contracts} contracts.")

                                # 3. Store (Optional)
                                count = 0
                                if chk_save_db.value:
                                    count = await run.io_bound(store_expired_options, parsed_options)
                                    log_view.push(f"  SUCCESS: Stored {count} contracts in DB.")
                                else:
                                    count = len(parsed_options)
                                    log_view.push(f"  SUCCESS: Processed {count} contracts (Skipped DB).")
                                
                                results.append({
                                    "expiry": expiry, 
                                    "status": "success", 
                                    "count": count,
                                    "candles": contracts_with_candles if chk_download_candles.value else "N/A"
                                })
                                
                            except Exception as e:
                                logger.error(f"Error processing {expiry}: {e}")
                                results.append({"expiry": expiry, "status": "error", "msg": str(e)})
                                log_view.push(f"  ERROR: {e}")
                        ...
                        # Show Summary
                        total_processed = sum(r.get('count', 0) for r in results if r['status'] == 'success')
                        ui.notify(f'Completed! Processed {total_processed} options.', type='positive')
                        
                        with ui.expansion(f'Download Report ({total_processed} contracts)', value=True).classes('w-full mt-2 bg-green-50'):
                            for res in results:
                                icon = '✅' if res['status'] == 'success' else '❌'
                                msg = f"{res.get('count', 0)} contracts" if res['status'] == 'success' else res.get('msg', 'Error')
                                ui.label(f"{icon} {res['expiry']}: {msg}").classes('ml-2')

                        # Trigger refresh of data view only if DB updated
                        if chk_save_db.value:
                            await refresh_data_view(symbol)

                    ui.button('Download Selected Option Chains', on_click=start_download, icon='cloud_download').props('color=green size=lg').classes('mt-4 w-full')
                    ...
                    async def refresh_data_view(sym):
                        ...
                        with data_container:
                            ...
                            stored_data = await run.io_bound(get_stored_expired_options, sym)
                            data_container.clear()
                            
                            if not stored_data:
                                ui.label("No data found in database.").classes('text-gray-500 italic')
                                return
                            ...
                            # Create a table
                            columns = [
                                {'name': 'expiry', 'label': 'Expiry Date', 'field': 'expiry', 'sortable': True, 'align': 'left'},
                                {'name': 'ce', 'label': 'Call (CE)', 'field': 'ce', 'sortable': True, 'align': 'center'},
                                {'name': 'pe', 'label': 'Put (PE)', 'field': 'pe', 'sortable': True, 'align': 'center'},
                                {'name': 'total', 'label': 'Total Contracts', 'field': 'total', 'sortable': True, 'align': 'right'},
                            ]
                            rows = [
                                {
                                    'expiry': k, 
