...
def render_page(state):
    Components.section_header('Option Greeks', 'Deep Dive Analysis (Delta, Theta, Gamma, Vega)', 'data_exploration')
    ...
    with ui.row().classes('w-full gap-4'):
        ...
        ui.input(label='Symbol', on_change=lambda e: current_symbol.update({'val': e.value})).props('outlined dense dark').classes('w-40').bind_value(current_symbol, 'val')
        ...
        async def fetch_greeks():
            ...
            with greeks_container:
                if not v3_data:
                    ui.label("V3 API returned no data. Using V2 Fallback if available.").classes('text-orange-400')
                    # Fallback to display V2 logic?? No, user wants V3.
                
                # Create Cards for each strike
                for s in selected:
                    with ui.card().classes('w-full bg-slate-800 p-2'):
                        ui.label(f"Strike: {s['strike']}").classes('text-lg font-bold text-center w-full mb-2 bg-slate-900 rounded')
                        
                        cols = '1fr 1fr 1fr 1fr 1fr'
                        with ui.grid(columns=cols).classes('w-full text-center text-xs gap-2'):
                            ...
                            # CE Row
                            ck = s['call']['instrument_key']
                            cd = v3_data.get(ck) # Keys might be formatted differently in response?
        ...
        ui.button('Analyze Greeks (ATM)', on_click=fetch_greeks).classes('bg-purple-600')
