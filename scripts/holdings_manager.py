...
class HoldingsManager:
            ...
            if data.get("status") == "success":
                ...
                for holding in holdings:
                    processed = {
                        ...
                        "current_value": holding.get("quantity", 0)
    ...
    def get_holdings_summary(self, holdings: List[Dict]) -> Dict:
        ...
        for h in holdings:
            ...
            if exchange not in by_exchange:
                by_exchange[exchange] = {"count": 0, "value": 0, "pnl": 0}
        ...
        sorted_by_pnl = sorted(
            holdings, key=lambda x: x["pnl_percentage"], reverse=True
        )
    ...
    def export_holdings(
        self, holdings: List[Dict], format: str = "json", filename: str = None
    ):
    ...
    def print_holdings(self, holdings: List[Dict]):
        ...
        print(
            f"{'Symbol':<10} {'Exchange':<8} {'Qty':>8} {'Avg Price':>12} "
        ...
        for h in holdings:
            ...
            print(
                f"{h['symbol']:<10} {h['exchange']:<8} {h['quantity']:>8} "
    ...
    def print_summary(self, summary: Dict):
        ...
        for exchange, data in summary["by_exchange"].items():
            print(
                f"   {exchange}: {data['count']} holdings, "
        ...
        for h in summary["top_gainers"]:
            print(
                f"   {h['symbol']}: +{h['pnl_percentage']:.2f}% " f"(₹{h['pnl']:,.2f})"
            )
...
def main():
    ...
    if args.action in ["get", "summary", "export", "metrics"]:
        ...
        print(
            f"{'Date':<12} {'Value':>15} {'Investment':>15} "
        ...
        for item in history:
            ...
            print(
                f"{item['date']:<12} ₹{item['total_value']:>14,.2f} "
