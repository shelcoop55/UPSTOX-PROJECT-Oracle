...
def create_new_tables(conn):
    ...
    cursor.executemany(
        "INSERT OR IGNORE INTO exchanges (id, name) VALUES (?, ?)", exchanges
    )
    ...
    cursor.executemany(
        "INSERT OR IGNORE INTO segments (id, exchange_id, type) VALUES (?, ?, ?)",
        segments,
    )
    ...
    cursor.executemany(
        "INSERT OR IGNORE INTO instrument_types (code, category, description) VALUES (?, ?, ?)",
        types,
    )
...
def migrate_data(conn):
    ...
    regex_opt = re.compile(
        r"([A-Z0-9]+)\s+(\d+(?:\.\d+)?)\s+(CE|PE)\s+(\d+\s+[A-Z]{3}\s+\d{2})"
    )
    ...
    for key, trading_symbol, type_code, underlying in derivatives:
        ...
        if len(rows_to_insert) > 1000:
            cursor.executemany(
                "INSERT OR IGNORE INTO derivatives_metadata (instrument_key, underlying_symbol, expiry_date, strike_price, option_type) VALUES (?, ?, ?, ?, ?)",
                rows_to_insert,
            )
            conn.commit()
    ...
    logger.info(
        f"Derivatives processed: {parsed_count} (Options: {options_count}, Futures: {futures_count})"
    )
