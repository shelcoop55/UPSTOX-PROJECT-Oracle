    ...
    def get_profile(self) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                logger.info(f"Profile fetched: {data.get('data', {}).get('user_name', 'Unknown')}")
    ...
    def get_positions(self) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Fetched positions: {len(positions.get('day', []))} day, {len(positions.get('net', []))} net")
                return positions
            else:
            ...
            for i in range(0, len(unique_keys), BATCH_SIZE):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(f"Batch quote fetch failed {response.status_code} for batch {i}")
