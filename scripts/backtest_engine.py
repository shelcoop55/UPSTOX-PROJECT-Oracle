...
class SimpleOptionSpreadStrategy(BaseStrategy):
    ...
    def generate_signals(self, ohlcv_data: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.warning("Bull call spread strategy requires strike data not yet implemented")
        return None
...
class BacktestEngine:
    ...
    def load_candle_data(
        ...
        try:
            ...
            logger.info(f"✓ Loaded {len(df)} candles for {symbol} from {df.index[0].date()} to {df.index[-1].date()}")
            return df
    ...
    def run_backtest(
        ...
        try:
            ...
            # Calculate CAGR
            duration_years = (backtest_data.index[-1] - backtest_data.index[0]).days / 365.25
            if duration_years > 0:
                cagr = (pf.final_value() / self.params['init_cash']) ** (1 / duration_years) - 1
            else:
                cagr = 0
            ...
            logger.info(f"  Return: {result.total_return*100:.2f}% | Sharpe: {result.sharpe_ratio:.2f} | Max DD: {result.max_drawdown*100:.2f}%")
            logger.info(f"  Trades: {result.total_trades} | Win Rate: {result.win_rate*100:.1f}%")
    ...
    def run_backtest_batch(
        ...
        for symbol in symbols:
            result = self.run_backtest(symbol, strategy, timeframe, start_date, end_date)
            if result:
    ...
    def print_results_summary(self):
        ...
        for result in self.results:
            ...
            logger.info(f"  Period: {result.start_date} to {result.end_date} ({result.duration_days} days)")
...
Examples:
  ...
  python3 scripts/backtest_engine.py --symbol INFY --strategy SMA \\
    ...
    parser.add_argument(
        '--symbol',
        type=str,
        help='Single symbol to backtest (e.g., INFY)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help='SMA fast period (default: 20)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help='SMA slow period (default: 50)'
    )
