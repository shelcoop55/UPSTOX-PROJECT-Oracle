...
if not TOKEN or TOKEN == "YOUR_TELEGRAM_BOT_TOKEN":
    logger.warning("TELEGRAM_BOT_TOKEN is missing or default. Bot will not start correctly.")
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send a help message."""
    await update.message.reply_text("Try asking: 'What are the top gainers today?' or 'Show my portfolio holdings'.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if len(response) > 4000:
            ...
            await status_msg.edit_text(response) # Parse mode markdown optional, stick to text for safety
            
    except Exception as e:
        logger.error(f"Error processing message: {e}")
        await status_msg.edit_text("Sorry, something went wrong while processing your request.")
