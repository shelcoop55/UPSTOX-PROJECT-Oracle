...
class StrategyRunner:
    ...
    def register_strategy(
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
        """,
            (name, description, json.dumps(parameters), max_positions, risk_per_trade),
        )
    ...
    def generate_rsi_signals(
        self, symbol: str, period: int = 14, oversold: int = 30, overbought: int = 70
    ...
    def _calculate_macd(
        self, prices: List[float], fast: int, slow: int, signal: int
    ) -> Tuple[float, float]:
    ...
    def execute_signal(self, signal: TradingSignal) -> Dict[str, Any]:
        ...
        if signal.confidence < 0.5:
            logger.info(
                f"Signal confidence too low ({signal.confidence:.2f}), skipping"
            )
        ...
        try:
            ...
            self._mark_signal_executed(
                signal_id, order_result["order_id"], order_result["price"]
            )
            ...
            return {"status": "FAILED", "signal_id": signal_id, "error": str(e)}
    ...
    def _mark_signal_executed(
        self, signal_id: int, order_id: str, execution_price: float
    ):
    ...
    def run_strategy(
        self, strategy_name: str, symbols: List[str]
    ) -> List[TradingSignal]:
        """
...
def main():
    ...
    parser.add_argument(
        "--strategy", type=str, default="RSI_Mean_Reversion", help="Strategy name"
    )
    parser.add_argument(
        "--symbols", type=str, default="INFY,TCS", help="Comma-separated symbols"
    )
    parser.add_argument(
        "--live", action="store_true", help="Run in live mode (default: dry run)"
    )
