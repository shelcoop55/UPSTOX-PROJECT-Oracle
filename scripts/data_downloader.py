...
class BaseDownloader:
    ...
    def validate_ohlc(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(invalid_close) > 0:
            logger.warning(f"Found {len(invalid_close)} rows with close outside high/low")
        ...
        cleaned_len = len(df)
        logger.info(f"Validated OHLC: {original_len} → {cleaned_len} rows ({original_len - cleaned_len} removed)")
        
        return df
    
    def detect_gaps(self, df: pd.DataFrame, expected_interval: str = '1D') -> List[Dict]:
        """
...
class StockDownloader(BaseDownloader):
    ...
    def get_instrument_key(self, symbol: str) -> str:
        ...
        logger.warning(f"Unknown instrument key for {symbol}, constructing NSE_EQ format")
        ...
        try:
            ...
            for candle in candles:
                ...
                records.append({
                    ...
                    'symbol': symbol.upper().split('|')[-1] if '|' in symbol else symbol.upper()
                })
    ...
    def download_and_process(
        ...
        if export_format:
            ...
            symbols_str = '_'.join(symbols) if len(symbols) <= 3 else f"{len(symbols)}_stocks"
            
            if export_format == 'parquet' or export_format == 'both':
                filepath = self.export_parquet(df, f"{symbols_str}_{interval}_{timestamp}.parquet")
            
            if export_format == 'csv' or export_format == 'both':
                csv_path = self.export_csv(df, f"{symbols_str}_{interval}_{timestamp}.csv")
                if not filepath:
                    filepath = csv_path
...
class OptionDownloader(BaseDownloader):
    ...
    def fetch_option_chain(
        ...
        logger.warning("Option chain download not yet implemented - requires Upstox auth")
        return pd.DataFrame()
...
def test_stock_download():
    ...
    logger.info(f"Test Results: {result['rows']} rows, {len(result['gaps'])} gaps, {result['validation_errors']} errors")
