...
class AuthManager:
    ...
    def save_token(self, user_id: str, token_data: Dict[str, any]):
        ...
        try:
            ...
            cursor.execute(
                "SELECT user_id FROM auth_tokens WHERE user_id = ?", (user_id,)
            )
        ...
        logger.info(
            f"✅ Token saved for user: {user_id} (expires: {datetime.fromtimestamp(expires_at)})"
        )
    ...
    def _refresh_token(
        self, user_id: str, refresh_token_encrypted: str
    ) -> Optional[str]:
        """
        ...
        try:
            refresh_token = self.cipher.decrypt(
                refresh_token_encrypted.encode()
            ).decode()
