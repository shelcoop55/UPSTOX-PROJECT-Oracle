...
class MultiExpiryLeg:
    ...
    def calculate_pnl(self, underlying_price: float, current_date: str) -> float:
        ...
        if current_date < self.expiry_date:
            ...
            days_to_expiry = (
                datetime.strptime(self.expiry_date, "%Y-%m-%d")
                - datetime.strptime(current_date, "%Y-%m-%d")
            ).days
            time_value_decay = (
                self.premium * 0.3 * (days_to_expiry / 30)
    ...
    def get_greeks(
        ...
        try:
            ...
            days_to_expiry = (
                datetime.strptime(self.expiry_date, "%Y-%m-%d") - datetime.now()
            ).days
            ...
            return {
                ...
                "price": max(
                    0, (S - K) if self.option_type == OptionType.CALL else (K - S)
                ),
            }
...
class MultiExpiryStrategy:
    ...
    def calculate_pnl(self, underlying_price: float, current_date: str) -> float:
        ...
        total_pnl = sum(
            leg.calculate_pnl(underlying_price, current_date) for leg in self.legs
        )
        return total_pnl
    ...
    def _find_breakevens(self, prices: List[float], pnls: List[float]) -> List[float]:
        ...
        for i in range(len(pnls) - 1):
            if pnls[i] * pnls[i + 1] < 0:  # Sign change
                ...
                breakeven = prices[i] + (prices[i + 1] - prices[i]) * abs(
                    pnls[i]
                ) / abs(pnls[i + 1] - pnls[i])
...
def create_diagonal_spread(
    ...
    return MultiExpiryStrategy(
        name=f"{option_type} Diagonal Spread", legs=legs, entry_price=underlying_price
    )


def create_double_calendar(
    underlying_price: float, near_expiry: str, far_expiry: str, qty: int = 50
) -> MultiExpiryStrategy:
    """
    ...
    legs = [
        # Put calendar
        MultiExpiryLeg(
            OptionType.PUT, ActionType.SELL, atm_strike, near_expiry, 75, qty
        ),
        MultiExpiryLeg(
            OptionType.PUT, ActionType.BUY, atm_strike, far_expiry, 110, qty
        ),
        # Call calendar
        MultiExpiryLeg(
            OptionType.CALL, ActionType.SELL, atm_strike, near_expiry, 75, qty
        ),
        MultiExpiryLeg(
            OptionType.CALL, ActionType.BUY, atm_strike, far_expiry, 110, qty
        ),
    ]
...
class ExpiryRoller:
    ...
    def should_roll(
        self, leg: MultiExpiryLeg, current_date: str, days_before_expiry: int = 3
    ) -> bool:
    ...
    def roll_position(
        ...
        days_to_new_expiry = (
            datetime.strptime(next_expiry, "%Y-%m-%d")
            - datetime.strptime(current_date, "%Y-%m-%d")
        ).days
        ...
        logger.info(
            f"Rolled position: {current_leg.expiry_date} → {next_expiry}, P&L: ₹{exit_pnl:.2f}"
        )
    ...
    def get_next_expiry(self, current_date: str, interval: str = "weekly") -> str:
        ...
        if interval == "weekly":
            ...
            # Find last Thursday
            days_in_month = (
                next_month.replace(month=next_month.month % 12 + 1, day=1)
                - timedelta(days=1)
            ).day
        ...
        for idx, row in hist_filtered.iterrows():
            ...
            if auto_roll:
                ...
                current_strategy = MultiExpiryStrategy(
                    strategy.name, rolled_legs, underlying_price
                )
