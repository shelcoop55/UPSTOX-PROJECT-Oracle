...
class PaperTradingSystem:
    ...
    def _init_portfolio(self):
        ...
        if count == 0:
            ...
            conn.commit()
            logger.info(
                f"Paper trading portfolio initialized: ₹{self.starting_capital:,.2f}"
            )
    ...
    def get_portfolio_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "return_pct": (
                (total_value - self.starting_capital) / self.starting_capital
            )
            * 100,
            "positions": positions,
        }
    ...
    def place_order(
        ...
        logger.info(
            f"Paper order placed: {order_id} - {transaction_type} {quantity} {symbol}"
        )
    ...
    def _get_position(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        if result:
            return {"symbol": symbol, "quantity": result[0], "average_price": result[1]}
    ...
    def update_portfolio_values(self):
        ...
        logger.debug(
            f"Portfolio values updated. Unrealized P&L: ₹{total_unrealized_pnl:,.2f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--type", type=str, choices=["BUY", "SELL"], help="Transaction type"
    )
    ...
    if args.action == "portfolio":
        ...
        if "order_id" in result:
            ...
            print(
                f"{result['transaction_type']} {result['quantity']} {result['symbol']}"
            )
        ...
        confirm = input(
            "Reset paper trading portfolio? This will delete all positions and orders. (yes/no): "
        )
