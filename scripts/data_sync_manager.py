...
class DataSyncManager:
    ...
    def register_sync_job(
        self, name: str, description: str, schedule_cron: str = "0 * * * *"
    ) -> int:
        """
    ...
    def detect_data_gaps(
        self, symbol: str, expected_interval_minutes: int = 5
    ) -> List[Dict]:
        """
                ...
                gaps.append(
                    {
                        ...
                        "missing_intervals": int(
                            gap_hours * 60 / expected_interval_minutes
                        ),
                    }
                )
    ...
    def sync_market_data(
        self, symbols: List[str], interval: str = "5minute"
    ) -> Dict[str, Any]:
        """
...
def main():
    ...
    if args.action == "register":
        ...
        for job in status["jobs"]:
            print(f"\n{job['job_name']}")
            print(
                f"  Status: {job['last_status']} ({'Enabled' if job['enabled'] else 'Disabled'})"
            )
            print(f"  Last Run: {job['last_run']}")
            print(f"  Success Rate: {job['success_rate']:.1f}%")
            print(
                f"  Runs: {job['run_count']} (✅ {job['success_count']}, ❌ {job['failure_count']})"
            )
