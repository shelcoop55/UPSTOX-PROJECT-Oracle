...
class EconomicCalendarFetcher:
    ...
    def _init_database(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_rbi_policy_date ON rbi_policy_decisions(meeting_date)"
        )
    ...
    def _load_static_events(self):
        ...
        for month in range(2, 13):
            pmi_date = datetime(2026, month, 3).strftime(
                "%Y-%m-%d"
            )  # Approx 3rd of month
    ...
    def display_calendar(
        self, events: List[Dict[str, Any]], title: str = "ECONOMIC CALENDAR"
    ):
        ...
        print("=" * 120)
        print(
            f"{'Date':<12} | {'Country':<8} | {'Event':<35} | {'Impact':<8} | {'Days Away':<10} | {'Category':<15}"
        )
        print("-" * 120)
        ...
        for event in events:
            ...
            print(
                f"{event_date:<12} | {country:<8} | {event_name:<35} | {impact_emoji} {impact:<6} | {days_str:<10} | {category:<15}"
            )
...
def main():
    ...
    parser.add_argument("--event-name", type=str, help="Event name for alert")
    ...
    if args.action == "calendar":
        ...
        success = fetcher.set_alert(
            event_name=args.event_name, days_before=args.days_before
        )
        ...
        fetcher.monitor_events(interval=args.interval, duration=args.duration)
