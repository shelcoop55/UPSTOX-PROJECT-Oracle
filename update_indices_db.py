...
def update_db():
    ...
    if df_50 is not None:
        ...
        for _, row in df_50.iterrows():
            sym = row.get('Symbol')
            if sym:
                cursor.execute("INSERT INTO stock_metadata (symbol, is_nifty50) VALUES (?, 1) ON CONFLICT(symbol) DO UPDATE SET is_nifty50=1", (sym,))
                count += 1
    ...
    if df_bank is not None:
        ...
        for _, row in df_bank.iterrows():
            sym = row.get('Symbol')
            if sym:
                cursor.execute("INSERT INTO stock_metadata (symbol, is_nifty_bank) VALUES (?, 1) ON CONFLICT(symbol) DO UPDATE SET is_nifty_bank=1", (sym,))
                count += 1
    ...
    # Verification
    print("\n--- Stats ---")
    print("Stocks with Sector:", cursor.execute("SELECT count(*) FROM stock_metadata WHERE sector IS NOT NULL").fetchone()[0])
    print("NIFTY 50 count:", cursor.execute("SELECT count(*) FROM stock_metadata WHERE is_nifty50=1").fetchone()[0])
    print("NIFTY 500 count:", cursor.execute("SELECT count(*) FROM stock_metadata WHERE is_nifty500=1").fetchone()[0])
